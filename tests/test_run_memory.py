from src.t0.memory import update_after_waypoint, RunMemory
...
def test_sanity_accumulated_from_node() -> None:
    mem = RunMemory()
    node = _node()
    node.sanity_lost_in_waypoint = 3
    update_after_waypoint(mem, node)
    assert mem.sanity == 3
...
def test_sanity_accumulates_across_multiple_nodes() -> None:
    ...
    for loss in (2, 1, 3):
        node = _node()
        node.sanity_lost_in_waypoint = loss
        update_after_waypoint(mem, node)
    assert mem.sanity == 6
...
def test_shock_increments_severity_and_dread() -> None:
    mem = RunMemory()
    node = _node()
    node.shocks_in_waypoint.append(ShockRecord(
...
def test_node_summary_appended_to_incidents() -> None:
    mem = RunMemory()
    node = _node()
    node.waypoint_summary = "crossroads:2_encounters:sanity=1"
    update_after_waypoint(mem, node)
...
def test_incidents_window_capped_at_five() -> None:
    ...
    for i in range(7):
        node = _node()
        node.waypoint_summary = f"summary_{i}"
        update_after_waypoint(mem, node)
