...
def test_overlay_effects_patches_three_stat_keys() -> None:
    ...
    eff = payload["options"][0]["metadata"]["effects"]
...
def test_overlay_effects_sets_toll_when_nonempty() -> None:
    payload = {"options": [_opt("a")]}
    _overlay_effects(payload, {"a": {"health_delta": 0, "money_delta": 0, "sanity_delta": 0, "toll": "destabilizing"}})
    assert payload["options"][0]["toll"] == "destabilizing"


def test_overlay_effects_skips_toll_when_empty_string() -> None:
    payload = {"options": [_opt("a")]}
    _overlay_effects(payload, {"a": {"health_delta": 0, "money_delta": 0, "sanity_delta": 0, "toll": ""}})
    assert "toll" not in payload["options"][0]


def test_overlay_effects_preserves_existing_metadata_keys() -> None:
    payload = {"options": [_opt("a", {"add_marks": ["cursed"], "effects": {}})]}
    _overlay_effects(payload, {"a": {"health_delta": 0, "money_delta": 0, "sanity_delta": -1, "toll": ""}})
    assert payload["options"][0]["metadata"]["add_marks"] == ["cursed"]


def test_overlay_effects_unknown_option_id_is_ignored() -> None:
    payload = {"options": [_opt("a")]}
    _overlay_effects(payload, {"z": {"health_delta": -99, "money_delta": 0, "sanity_delta": 0, "toll": ""}})
    assert "effects" not in payload["options"][0]["metadata"]


def test_overlay_effects_multiple_options_patches_only_matching() -> None:
    payload = {"options": [_opt("a"), _opt("b")]}
    _overlay_effects(payload, {"b": {"health_delta": 3, "money_delta": 0, "sanity_delta": 0, "toll": ""}})
    assert "effects" not in payload["options"][0]["metadata"]
    assert payload["options"][1]["metadata"]["effects"]["health_delta"] == 3
