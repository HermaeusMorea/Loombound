...
from src.t2.core.collector import build_a1_entry, _band, _direction
...
def test_pressure_level_critical_when_sanity_very_low() -> None:
    ...
    entry = build_a1_entry(_core(sanity=1), RunMemory(), _node())
...
def test_pressure_level_low_when_sanity_high() -> None:
    ...
    entry = build_a1_entry(_core(sanity=75), RunMemory(), _node())
    assert entry.pressure_level == "low"


# ---------------------------------------------------------------------------
# build_a1_entry — resource_trajectory
# ---------------------------------------------------------------------------

def test_trajectory_critical_when_heavy_sanity_loss() -> None:
    entry = build_a1_entry(_core(), RunMemory(), _node(sanity_lost=3))
    assert entry.resource_trajectory == "critical"


def test_trajectory_depleting_when_moderate_loss() -> None:
    entry = build_a1_entry(_core(), RunMemory(), _node(sanity_lost=2))
...
# ---------------------------------------------------------------------------
# build_a1_entry — passthrough fields
# ---------------------------------------------------------------------------
...
def test_entry_waypoint_id_and_depth_match_node_memory() -> None:
    ...
    entry = build_a1_entry(_core(), RunMemory(), node)
