...
| **A1** | `scene_concept`、选项骨架（label、意图描述）；A1 cache table 内容；saga waypoint 图（拓扑、`next_waypoints`）；`map_blurb`；`narration_templates.json` |
...
| **C3** | claude-opus-4-6 | A3（创作意图） | A2（cache table、toll lexicon）+ A1（saga 图、waypoint 结构） | 离线（`gen`） |
...
| **A1 cache table** | `data/nodes/<id>/a1_cache_table.json` | 每次 gen（C2） | per-saga | per-waypoint 场景骨架：scene_concept、选项结构、effects |
...
| **toll lexicon** | saga 专属 toll 词汇表，C3 生成，存为 `data/campaigns/<id>_toll_lexicon.json`，运行时附在 A1 option index 后缓存 |
...
| **gen** | `./loombound gen "theme"` | C3 + C2 | `data/campaigns/<id>.json` + `data/campaigns/<id>_toll_lexicon.json` + `data/nodes/<id>/a1_cache_table.json` |
...
| `src/t3/memory/` | A3 | saga 图、tone 等数据模型 |
| `src/t3/core/` | C3 | saga 生成逻辑 |
| `src/runtime/` | 组装点 | play_cli、session、saga 加载；唯一可 import 全部层的位置 |
