...
| 外周神经 + 肌肉 | 纯局部执行、无网络 | C1 — qwen2.5:7b 本地（场景展开） | 受硬件限制 |
...
**C1 延迟的硬件根源：** C1 当前的 16–37 秒延迟不是模型本身慢，而是作者本地硬件不足、回落到 CPU 推理的结果。qwen2.5:7b 全量加载约需 4–5 GB 显存，有 GPU 时可达 20–80 token/s（~2–10 秒/场景），无 GPU 时回落到 CPU 推理，速度降 5–10×。硬件到位后，C1 延迟将与 C2 的 1–2 秒处于同一数量级，"肌肉反射"的类比才能完全成立。
